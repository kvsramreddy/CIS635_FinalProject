...
seasonalityData=decompose(ts_consumption, type = c("multiplicative"))
seasonalityData
plot(seasonalityData)
seasonallyAdjusted<-(seasonalityData$x-seasonalityData$seasonal)
plot(seasonallyAdjusted)
plot(seasonalityData$seasonal)
...
Hw %>%
  forecast(h=60) %>%
  autoplot() + autolayer(electricity_split$test)
...
tsMod <-arima(x=electricity_split$train, order=c(0.7,1,0.25))
head(tsMod)
print(tsMod)

```
```{r}
autoModel<-auto.arima(electricity_split$train)
autoModel
autoPredict<-forecast(autoModel, h=60)
autoPredict
accuracy(autoPredict, electricity_split$test)
```

```{r}
# Forecasting
prediction<-forecast(tsMod, h=60)
prediction
```

```{r}
# Plot the final series
autoplot(prediction)+autolayer(electricity_split$test)
```

```{r comparing accuracy}
accuracy(next_Year, electricity_split$test)
accuracy(prediction, electricity_split$test)
#accuracy(autoPredict, electricity_split$test)
```
...
```{r}
hwAddModel<-hw(electricity_split$train, seasonal="additive")
hwMulModel<-hw(electricity_split$train, seasonal="multiplicative")
accuracy(hwAddModel)
accuracy(hwMulModel)
```

```{r}
accuracy(predict(hwAddModel),electricity_split$test)
accuracy(predict(hwMulModel),electricity_split$test)
autoplot(hwAddModel)
autoplot(hwMulModel)
```

```{r}
hwAddModel$residuals
hwMulModel$residuals
```